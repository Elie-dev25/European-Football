...
# Colonnes bloquantes si NULL : la jointure doit garantir leur présence
CRITICAL_COLUMNS = {"HOME_TEAM_NAME", "AWAY_TEAM_NAME"}
# Colonnes tolérées : NULL légitime en source (ex. FIFA ID parfois absent)
# -> tout ce qui n'est pas dans CRITICAL_COLUMNS est toléré par défaut


def check_match_core_quality():
    with get_snowflake_connection() as conn:
        raw_count = get_row_count(conn, DATABASE, "RAW", "MATCH")
        silver_count = get_row_count(conn, DATABASE, SCHEMA, TABLE)
        duplicates = get_duplicate_count(conn, DATABASE, SCHEMA, TABLE, "MATCH_API_ID")
        null_counts = get_null_counts(conn, DATABASE, SCHEMA, TABLE)

    print(f"RAW.MATCH : {raw_count} lignes")
    print(f"{SCHEMA}.{TABLE} : {silver_count} lignes")
    print(f"Complétude OK : {raw_count == silver_count}")
    print(f"Doublons sur MATCH_API_ID : {duplicates}")
    print()
...
if __name__ == "__main__":
    import sys
    success = check_match_core_quality()
