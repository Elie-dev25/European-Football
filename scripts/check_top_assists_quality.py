...
CRITICAL_COLUMNS = {"LEAGUE_ID", "SEASON", "TEAM_ID", "PLAYER_ID", "GOALS_ASSISTS"}


def check_top_assists_quality():
    with get_snowflake_connection() as conn:
        raw_count = get_row_count(
            conn, DATABASE, "RAW", "TOP_ASSISTS",
            where="""team_id IN (
        ...
        duplicate_count = get_duplicate_count(
            conn, DATABASE, SCHEMA, TABLE, ["LEAGUE_ID", "SEASON", "TEAM_ID", "PLAYER_ID"]
        )

    print(f"RAW.TOP_ASSISTS (filtré aux équipes de RAW.FIXTURES) : {raw_count} lignes")
    print(f"{SCHEMA}.{TABLE} : {silver_count} lignes")
    print(f"Complétude OK : {raw_count == silver_count}")
    print(f"Doublons sur (LEAGUE_ID, SEASON, TEAM_ID, PLAYER_ID) : {duplicate_count}")
    print()
...
if __name__ == "__main__":
    import sys
    success = check_top_assists_quality()
